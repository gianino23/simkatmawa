...
class SlideController extends Controller
{
		...
		for($index = 0;$index < $countfiles;$index++){
		   ...
		   if(isset($_FILES['files']['name'][$index]) && $_FILES['files']['name'][$index] != ''){
			  ...
			  if(in_array($ext, $valid_ext)){
				...
				$model1 =new Slide ;
				$model1->judul=$_POST['modal_judul'];
				$model1->subjudul=$_POST['modal_subjudul'];
				$model1->urutan=$_POST['modal_urutan'];
				$model1->status=$_POST['modal_status'];
				$model1->file=$filename;
				$model1->save();
			  }
		   }
		}
		...
		if($model1->save())
		 {
		  ...
		  $content = file_get_contents(Yii::app()->createAbsoluteUrl('slide/adminn'), true);
	...
	{
		$id=$_POST['id_slide'];
		$model1=Slide::model()->findAllByAttributes(array('id_slide'=>$id));
		if(count($model1)==1){
			$model2=Slide::model()->findByAttributes(array('id_slide'=>$id));
			unlink(Yii::app()->basePath . '/../upload/slide/'.$model2->file);
		}else{
		foreach($model1 as $mdl){
		unlink(Yii::app()->basePath . '/../upload/slide/'.$mdl->file);
		}
		}
