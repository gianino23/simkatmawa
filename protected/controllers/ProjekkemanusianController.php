...
class ProjekkemanusianController extends Controller
{
	/**
	...
	public function actionTambah()
	{
		...
		$model=new Projekkemanusian;
		
		$model->nama_program = $_POST['modal_program'];
		$model->judul_kegiatan = $_POST['modal_judul'];
		...
		for($index = 0;$index < $countfiles;$index++){
		   ...
		   if(isset($_FILES['files']['name'][$index]) && $_FILES['files']['name'][$index] != ''){
			  ...
			  if(in_array($ext, $valid_ext)){
				...
				$model1 =new Fileprojekkemanusian ;
				$model1->projekkemanusian_id=$model->id_projekkemanusian;
		...
		if($model->save())
		 {
		  ...
		  $content = file_get_contents(Yii::app()->createAbsoluteUrl('projekkemanusian/adminn'), true);
	...
	public function actionEdit($id)
	{
		...
		if(isset($_POST['Projekkemanusian']))
		{
			$model->attributes=$_POST['Projekkemanusian'];
			if($model->save())
				$this->redirect(array('view','id'=>$model->id_projekkemanusian));
		}
	...
	{
		$dataProvider=new CActiveDataProvider('Projekkemanusian');
	...
	{
		$model=new Projekkemanusian('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Projekkemanusian']))
			$model->attributes=$_GET['Projekkemanusian'];
	 ...
	 * @return Projekkemanusian the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=Projekkemanusian::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param Projekkemanusian $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='projekkemanusian-form')
		{
